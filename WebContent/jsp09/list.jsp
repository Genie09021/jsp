...
<%
	...
	// 전체 글의 개수 가져오기 
	int count = dao.getArticleCount();   //던져줄 변수 없어도됨 DB에 저장되어있는 전체 글의 개수를 가져와 담기
	System.out.println("count : " + count);
	// 글이 하나라도 있으면 글들을 다시 가져오기 
	List articleList = null; // 밖에서 사용가능하게 if문 시작 전에 미리 선언 
	if(count > 0){
		articleList = dao.getArticles(startRow, endRow); //글을 싹다가져오는것이아니고 startRow 부터 end까지 가져오기
	}
	int number = count - (currentPage-1)*pageSize; 	// 게시판 목록에 뿌려줄 가상의 글 번호  .db에는 글의 고유번호가 저장되기때문에 글이삭제되거나 해도 번호가바뀌지않음으로
	//페이지에서는 순서대로 바뀌게 .
